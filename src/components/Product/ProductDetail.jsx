...
import { Layout, Row, Col, Button, InputNumber, Form } from "antd";
...
export default function ProductDetail() {
  ...
  let categoriesLink = categories?.map((item) => {
    return (
      <Link key={item?.id} to={`/danh-muc/${item?.attributes?.slug}`}>
            ...
            <Col xs={24} md={12} className="info">
              ...
              {categoriesLink ? (
                <div className="categories-link">
                  <span className="label-field">Danh mục: </span>{" "}
                  {categoriesLink}
                </div>
              ) : null}
              {brand ? (
                <div>
                  ...
                  <Link to={`/tim?brand=${brand}`}>{brand}</Link>
                </div>
              ) : null}
              {data?.attributes?.cpu ? (
                <div>
              ...
              {data?.attributes?.quantityAvailable ? (
                <>
                  ...
                  <div className="btn-wrapper">
                    <Button className="btn" type="primary" onClick={addToCart}>
                      Thêm giỏ hàng
                    </Button>
                  </div>
                </>
